...
log_transformer = FunctionTransformer(func=log_transform, inverse_func=exp_transform)
...
def objective(trial):
    ...
    model = Pipeline(steps=[('preprocessor', preprocessor),
                            ('regressor',  TransformedTargetRegressor(regressor=SVR(C=trial.suggest_float('C',10, 200),
                                                                       kernel=trial.suggest_categorical('kernel', ['linear', 'poly', 'rbf',]),
                                                                       epsilon=trial.suggest_float('epsilon', 0.01,1),
                                                                       degree=2,), transformer=log_transformer),),
...
best_model = Pipeline(steps=[('preprocessor', preprocessor),
                             ('regressor', TransformedTargetRegressor(regressor=SVR(C=best_params['C'],
...
#save best model
dump(best_model, 'best_model_vkernel_exp.joblib')


#{'alpha': 0.011127786242354887, 'kernel': 'poly', 'gamma': 0.09788295528991368, 'degree': 3}