...
def Plot_Q5(app):
    ...
    def validate_input_and_update_prediction(n_clicks, model, future_year, locabbr, race, gender):
        #print(n_clicks)
        if n_clicks is None:
            # No button click yet
            return '', False
        ...
        if all(param is not None and param != '' for param in [model, future_year, locabbr, race, gender]):
            ...
            if future_year.isnumeric() and len(future_year) == 4:
                ...
                prediction = round(y_pred(model, future_year, locabbr, race, gender)[0])  # Replace with your actual prediction logic
